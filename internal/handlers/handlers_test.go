...
func TestAdminProcessReservation(t *testing.T) {
	for _, e := range adminProcessReservationTests {
		...
		// Add form values that the handler expects
		req.PostForm = url.Values{}
		if strings.Contains(e.queryParams, "y=") && strings.Contains(e.queryParams, "m=") {
			year := strings.Split(strings.Split(e.queryParams, "y=")[1], "&")[0]
			month := strings.Split(e.queryParams, "m=")[1]
			req.PostForm.Add("year", year)
			req.PostForm.Add("month", month)
		}

		// Make sure the form is parsed
		req.Header.Set("Content-Type", "application/x-www-form-urlencoded")

		ctx := getCtx(req)
		req = req.WithContext(ctx)
		rr := httptest.NewRecorder()
		handler := http.HandlerFunc(Repo.AdminPostProcessReservation)
		handler.ServeHTTP(rr, req)
