...
func TestRepository_PostReservation(t *testing.T) {
	...
	// Create a new HTTP POST request with the encoded request body
	req, _ := http.NewRequest("POST", "/make-reservation", strings.NewReader(postedData.Encode()))

	// Get a context that includes session data and attach it to the request
	ctx := getCtx(req)
	req = req.WithContext(ctx)

	// Set the content type to match a typical form submission
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")

	// Create a response recorder to capture the response
	rr := httptest.NewRecorder()

	// Define the handler function that we are testing
	handler := http.HandlerFunc(Repo.PostReservation)

	// Serve the request and record the response
	handler.ServeHTTP(rr, req)

	// Assert that the response code is 303 See Other (indicating a redirect)
